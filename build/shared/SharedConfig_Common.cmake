...
function(AddLibraryAndDependencies targetName staticBuild isFramework sharedOrModule sourceFiles dependencyList)
    ...
    foreach(dependencyPair ${${dependencyList}})
        if (${dependencyPair} STREQUAL "ZLIB")
            ...
            target_link_libraries(${targetName} PUBLIC ZLIB::ZLIB)
        else()
            find_package(${dependencyPair} REQUIRED CONFIG)
            if (TARGET ::${dependencyPair})
                target_link_libraries(${targetName} PUBLIC ::${dependencyPair})
            elseif(TARGET ${dependencyPair}::${dependencyPair})
                target_link_libraries(${targetName} PUBLIC ${dependencyPair}::${dependencyPair})
            else()
                message(FATAL_ERROR "dependencyPair ${dependencyPair} not found")
            endif()
        endif()
